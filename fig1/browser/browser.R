...
plot.reads=function(reads,start,y=0,height=1,bin=1,ymax=NULL,col='#000000')
{
    data=reads
    n=length(data)
    data.max = max(c(data, 0))
    data.min = min(c(data, 0))
    if(data.max == 0 & data.min == 0) return(0)
    
    if(!is.null(ymax)) {
      if(data.max == 0) data.min = -ymax
      else if(data.min == 0) data.max = ymax
      else if(data.max >= -data.min) data.max = ymax
      else if(-data.min > data.max) data.min = ymax
    }
    
    data.range = data.max - data.min
    
    data = (data - data.min) / data.range
    data.0 = (0 - data.min) / data.range
    data[data > 1] = 1
    data[data < 0] = 0
    
    if(is.null(ymax)) ymax=max(c(data.max, -data.min))

    rect((1:n-1)*bin+start,y+data.0*height*0.95,(1:n)*bin+start,y+data*height*0.95 ,col=col,border=NA)
    return(ymax)
}
...
plot.browser=function(reads,genes,start,end,bin=1,col=NULL,gene.col=NULL,y=NULL,height=NULL,ymax=NULL,vline=NULL,line.gene=1)
{
  ...
  if(is.null(ymax)) for(i in 1:n) max[i]=plot.reads(reads[,i],start,y=y[i],bin=bin,col=col[i],height=height[i])
	...
	if(length(genes)>0) for(i in 1:length(genes))
    {
		...
		if(gene.rx.end < gene.rx.start) next
		gene.Disp.line = 0
...
browser.setup=function(genelist=NULL,bedgraph=NULL,description=NULL,heights=NULL,col=NULL,hlines=NULL,label.y=NULL,readcount=NULL,gene.line=1)
{
    ...
    browser.param$gene.line<<-gene.line 
}
...
browser.setgene=function(genename, margin = 0.1)
{
    g=find.gene(browser.param$genelist,genename)
    if(!is.na(g$name))
    {
        length=g$end-g$start
        browser.setpos(g$chr,floor(g$start-length * margin),ceiling(g$end+length*margin))
    }
}
